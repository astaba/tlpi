/* =========================================================================
 * Created on: <Tue Aug 04 17:47:32 +01 2026>
 * Time-stamp: <Tue Aug  4 17:51:29 +01 2026 by owner>
 * Author    : Copyright (C) Michael Kerrisk, 2026.
 *             See the file [file:../COPYING.gpl-v3] for details.
 * Desc      : ~/coding/c_prog/tlpi/proc/modify_env.c -
 *
 * Listing 6.4: Demonstrate modification of the process environment
 * list.
 *
 * Usage: modify_env name=value...
 *
 * NOTE: some UNIX implementations do not provide clearenv(),
 * setenv(), and unsetenv().
 * ========================================================================= */
#define _GNU_SOURCE /* Get various declarations from <stdlib.h> */
#include "tlpi_hdr.h"
#include <stdlib.h>

extern char **environ;

int main(int argc, char *argv[]) {
  int j;
  char **ep;

  clearenv(); /* Erase entire environment */

  /* Add any definitions specified on command line to environment */

  for (j = 1; j < argc; j++)
    if (putenv(argv[j]) != 0)
      systmErr("putenv: %s", argv[j]);

  /* Add a definition for GREET if one does not already exist */

  if (setenv("GREET", "Hello world", 0) == -1)
    systmErr("setenv");

  /* Remove any existing definition of BYE */

  unsetenv("BYE");

  /* Display current environment */

  for (ep = environ; *ep != NULL; ep++)
    puts(*ep);

  exit(EXIT_SUCCESS);
}
