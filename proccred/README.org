...
#+DESCRIPTION: Chapter 9 The Linux Programming Interface by Michael Kerrisk
#+STARTUP: showall

* Table Of Contents                                                   :toc_2:
- [[#91-real-user-id-and-real-group-id][9.1 Real User ID and Real Group ID]]
- [[#92-effective-user-id-and-effective-group-id][9.2 Effective User ID and Effective Group ID]]
- [[#93-set-user-id-and-set-group-id-programs][9.3 Set-User-ID and Set-Group-ID Programs]]
- [[#94-saved-set-user-id-and-saved-set-group-id][9.4 Saved Set-User-ID and Saved Set-Group-ID]]
- [[#95-file-system-user-id-and-file-system-group-id][9.5 File-System User ID and File-System Group ID]]
- [[#96-supplementary-group-ids][9.6 Supplementary Group IDs]]
- [[#97-retrieving-and-modifying-process-credentials][9.7 Retrieving and Modifying Process Credentials]]
  - [[#971-retrieving-and-modifying-real-effective-and-saved-set-ids][9.7.1 Retrieving and Modifying Real, Effective, and Saved Set IDs]]
  - [[#972-retrieving-and-modifying-file-system-ids][9.7.2 Retrieving and Modifying File-System IDs]]
  - [[#973-retrieving-and-modifying-supplementary-group-ids][9.7.3 Retrieving and Modifying Supplementary Group IDs]]
  - [[#974-summary-of-calls-for-modifying-process-credentials][9.7.4 Summary of Calls for Modifying Process Credentials]]
  - [[#975-example-displaying-process-credentials][9.7.5 Example: Displaying Process Credentials]]
- [[#98-summary][9.8 Summary]]
- [[#99-exercises][9.9 Exercises]]

* 9.1 Real User ID and Real Group ID
:PROPERTIES:
:CUSTOM_ID: 91-real-user-id-and-real-group-id
:END:

* 9.2 Effective User ID and Effective Group ID
:PROPERTIES:
:CUSTOM_ID: 92-effective-user-id-and-effective-group-id
:END:

* 9.3 Set-User-ID and Set-Group-ID Programs
:PROPERTIES:
:CUSTOM_ID: 93-set-user-id-and-set-group-id-programs
:END:

* 9.4 Saved Set-User-ID and Saved Set-Group-ID
:PROPERTIES:
:CUSTOM_ID: 94-saved-set-user-id-and-saved-set-group-id
:END:

* 9.5 File-System User ID and File-System Group ID
:PROPERTIES:
:CUSTOM_ID: 95-file-system-user-id-and-file-system-group-id
:END:

* 9.6 Supplementary Group IDs
:PROPERTIES:
:CUSTOM_ID: 96-supplementary-group-ids
:END:

* 9.7 Retrieving and Modifying Process Credentials
:PROPERTIES:
:CUSTOM_ID: 97-retrieving-and-modifying-process-credentials
:END:

** 9.7.1 Retrieving and Modifying Real, Effective, and Saved Set IDs
:PROPERTIES:
:CUSTOM_ID: 971-retrieving-and-modifying-real-effective-and-saved-set-ids
:END:

** 9.7.2 Retrieving and Modifying File-System IDs
:PROPERTIES:
:CUSTOM_ID: 972-retrieving-and-modifying-file-system-ids
:END:

** 9.7.3 Retrieving and Modifying Supplementary Group IDs
:PROPERTIES:
:CUSTOM_ID: 973-retrieving-and-modifying-supplementary-group-ids
:END:

** 9.7.4 Summary of Calls for Modifying Process Credentials
:PROPERTIES:
:CUSTOM_ID: 974-summary-of-calls-for-modifying-process-credentials
:END:

** 9.7.5 Example: Displaying Process Credentials
:PROPERTIES:
:CUSTOM_ID: 975-example-displaying-process-credentials
:END:

* 9.8 Summary
:PROPERTIES:
:CUSTOM_ID: 98-summary
:END:

* 9.9 Exercises
:PROPERTIES:
:CUSTOM_ID: 99-exercises
:END:
                   PROCESS CREDENTIALS
...
runnable by any user by making it a set-user-ID-root program, as follows:
    ...
    # chmod u+s check_password                  With the set-user-ID bit enabled
                                                                        ...
                                                                        P r oc e s s C re d e n t i als   169
                  # ls -l check_password
...
9.5   File-System User ID and File-System Group ID
      ...
      we’ll generally describe various file permission checks, as well as the setting of the
                                                                          ...
                                                                          P r oc e s s C re d e n t i als   171
...
similar, but with setgid() substituted for setuid() and group for user. With these
                                                                    ...
                                                                    P r oc e s s C re d e n t i als   173
...
1.   An unprivileged process can set the real user ID only to the current value of
     ...
     or saved set-user-ID.
                                                                         ...
                                                                         P r oc e s s C re d e n t i als   175
                          SUSv3 says that it is unspecified whether an unprivileged process can use
           ...
           3.     For both privileged and unprivileged processes, the saved set-user-ID is also set
...
that change multiple identifiers.)
                                                                     ...
                                                                     P r oc e s s C re d e n t i als   177
...
plementary group IDs.
    ...
    int initgroups(const char *user, gid_t group);
                                              Both return 0 on success, or –1 on error
                                                                    ...
                                                                    P r oc e s s C re d e n t i als   179
...
9.7.4        Summary of Calls for Modifying Process Credentials
             ...
             the user IDs, but the rules for changes to the group IDs are similar.
                     ...
                     setuid(u)                 seteuid(e)                 setreuid(r, e)                     setresuid(r, e, s)
                      ...
                      user ID                                user ID                                           set-user-ID


                                 has effect only for privileged processes            if r != –1 or e != previous-real,
                                                                                     then saved set-user-ID is
                    r, e, s      has effect for all processes; r, e, s,              made same as e
...
Table 9-1: Summary of interfaces used to change process credentials
 ...
 setuid(u)            Change effective ID to the    Change real,         Specified in SUSv3;
 setgid(g)            same value as current real    effective, and       BSD derivatives
                      or saved set ID               saved set IDs to     have different
                                                    any (single) value   semantics
 seteuid(e)           Change effective ID to the    Change effective     Specified in SUSv3
 setegid(e)           same value as current real    ID to any value
                      or saved set ID
 setreuid(r, e)       (Independently) change        (Independently)      Specified in SUSv3,
 setregid(r, e)       real ID to same value as      change real and      but operation
                      current real or effective     effective IDs to     varies across
                      ID, and effective ID to       any values           implementations
                      same value as current real,
                      effective, or saved set ID
 setresuid(r, e, s)   (Independently) change        (Independently)      Not in SUSv3 and
 setresgid(r, e, s)   real, effective, and saved    change real,         present on few
                      set IDs to same value as      effective, and       other UNIX
                      current real, effective, or   saved set IDs to     implementations
                      saved set ID                  any values
 setfsuid(u)          Change file-system ID to      Change file-system   Linux-specific
 setfsgid(u)          same value as current real,   ID to any value
                      effective, file system, or
                      saved set ID
 setgroups(n, l)      Can’t be called from an       Set supplementary    Not in SUSv3, but
                      unprivileged process          group IDs to any     available on all UNIX
...
z    Calls to setresuid() always modify the file-system user ID to have the same value
     ...
     by the call. Calls to setresgid() have an analogous effect on the file-system group ID.
                                                                         ...
                                                                         P r oc e s s C re d e n t i als   181
9.7.5        Example: Displaying Process Credentials
...
9.8   Summary
      ...
      process user and group IDs).
                                                                      ...
                                                                      P r oc e s s C re d e n t i als   183
9.9        Exercises
  9-1.     Assume in each of the following cases that the initial set of process user IDs is
  ...
  9-5.     Repeat the previous exercise for a process executing a set-user-ID-root program,
