...
- The =sa_restore= field: is not intended for application usage.
...
terminates the process). The process is relegated in the scheduler
...
- [[file:signal.c][Listing 22.1 signal.c]].
- [[file:signal_functions.c::75][signal_functions.c]], from within the =tlpi/lib= folder, when building
  ...
  some System V derivatives). Check static library /symbols/ with =nm
  -A <static_lib> | grep <symbol>=.
...
An alternative is to write our own version of =signal()= ([[file:signal_functions.c::75][Signal()]]),
probably similar to Listing 22.1, specifying exactly the flags that we
require, and employ that version with our applications.

 Note, however, that it is portable (and shorter) to use =signal()= to
 set the disposition of a signal to =SIG_IGN= or =SIG_DFL=, and we’ll
