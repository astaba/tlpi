...
/**
 * Signal - A wrapper for sigaction that provides portable signal
 * handling on Posix-compliant systems.
 *
 * Compile with -DOLD_SIGNAL to get the older, unreliable signal
...
void (*Signal(int sig, void (*handler)(int)))(int) {
  ...
  (void)sigemptyset(&newDisp.sa_mask);
#ifdef OLD_SIGNAL
