...
parse_command_options(int argc, char *argv[], struct cmd_options *opts)
{
    ...
    while ((opt = getopt(argc, argv,
                         "+hCimnuUM:G:P:rDvpAIs:b:S:dx:X:w:")) != -1) {
        switch (opt) {
    ...
    if ((opts->uid_map != NULL || opts->gid_map != NULL ||
            opts->projid_map != NULL || opts->create_root_mappings) &&
            !(opts->flags & CLONE_NEWUSER)) {
        fprintf(stderr, "-M, -G, -P, or -r specified without -U\n");
        referToHelp(argv[0]);
    }

    /* -M or -G with -r is nonsensical */

    if (opts->create_root_mappings &&
                (opts->uid_map != NULL || opts->gid_map != NULL)) {
        fprintf(stderr, "Can't combine -r with -M or -G\n");
        referToHelp(argv[0]);
    }
