...
#+DESCRIPTION: Chapters 05 from "The Linux Programming Interface" A Linux and UNIX® System Programming by Handbook Michael Kerrisk.
#+STARTUP: showall

* Table Of Contents                                                   :toc_2:
- [[#51-atomicity-and-race-conditions][5.1 Atomicity and Race Conditions]]
  - [[#creating-a-file-exclusively][Creating a file exclusively]]
- [[#53-open-file-status-flags][5.3 Open File Status Flags]]
- [[#57-scatter-gather-io-readv-and-writev][5.7 Scatter-Gather I/O: =readv()= and =writev()=]]
  - [[#scatter-input][Scatter input]]
- [[#510-io-on-large-files][5.10 I/O on Large Files]]
  - [[#the-transitional-lfs-api][The transitional LFS API]]
- [[#file-io-further-details][FILE I/O: FURTHER DETAILS]]
- [[#51-atomicity-and-race-conditions-1][5.1 Atomicity and Race Conditions]]
- [[#52-file-control-operations-fcntl][5.2 File Control Operations: fcntl]]
- [[#53-open-file-status-flags-1][5.3 Open File Status Flags]]
- [[#54-relationship-between-file-descriptors-and-open-files][5.4 Relationship Between File Descriptors and Open Files]]
- [[#55-duplicating-file-descriptors][5.5 Duplicating File Descriptors]]
- [[#56-file-io-at-a-specified-offset-pread-and-pwrite][5.6 File I/O at a Specified Offset: pread() and pwrite]]
- [[#57-scatter-gather-io-readv-and-writev-1][5.7 Scatter-Gather I/O: readv() and writev]]
- [[#58-truncating-a-file-truncate-and-ftruncate][5.8 Truncating a File: truncate() and ftruncate]]
- [[#59-nonblocking-io][5.9 Nonblocking I/O]]
- [[#510-io-on-large-files-1][5.10 I/O on Large Files]]
- [[#511-the-devfd-directory][5.11 The /dev/fd Directory]]
- [[#512-creating-temporary-files][5.12 Creating Temporary Files]]
- [[#513-summary][5.13 Summary]]
- [[#514-exercises][5.14 Exercises]]

* 5.1 Atomicity and Race Conditions

** Creating a file exclusively

- [[file:bad_exclusive_open.c][Listing 5.1 bad_exclusive_open.c]]: Incorrect code to exclusively open
  a file.
- [[file:ybadoexcl.c][ybadoexcl.c]]: Practice draft for bad_exclusive_open.c.
- [[file:yoexcl.c][yoexcl.c]]: Practice draft for good exclusive open.

* 5.3 Open File Status Flags

- [[file:yfile_flags.c][yfile_flags.c]]: Personal Practice for Using the =fcntl(2)= to get
...
** Scatter input
- [[file:t_readv.c][Listing 5.2 t_readv.c]]: Performing scatter input with readv();

* 5.10 I/O on Large Files

** The transitional LFS API

A) [[file:large_file.c][Listing 5.3 large_file.c]]: Accessing large files.

   - The Manual Way (=_LARGEFILE64_SOURCE=): You have to explicitly
     use =off64_t= and =lseek64()=. This is tedious and "infects" your
     whole codebase with "64" suffixes.

B) [[file:ylarge_file.c][ylarge_file.c]]: Demonstrate the use of the (modern) Large File
   Summit (LFS).

   - The Transparent Way (=_FILE_OFFSET_BITS=64=): This is the modern
     standard. It tells the preprocessor to "alias" the standard
     functions. When you type =lseek()=, the compiler secretly
     replaces it with the 64-bit version. Your code stays clean and
     portable.

C) The /Large File Summi/t or *LFS* created the /...64/ functions as a
   "transitional" step while the world was moving from 32-bit to
   64-bit.  On a 64-bit machine: This is all irrelevant, like =off_t=
   is already 64 bits by default.  On a 32-bit machine: Using
   =_FILE_OFFSET_BITS=64= ensures your code works the same way it does
   on a 64-bit machine without you changing a single line of C code.
...
* 5.1 Atomicity and Race Conditions
:PROPERTIES:
:CUSTOM_ID: 51-atomicity-and-race-conditions
:END:
* 5.2 File Control Operations: fcntl
:PROPERTIES:
:CUSTOM_ID: 52-file-control-operations-fcntl
:END:
* 5.3 Open File Status Flags
:PROPERTIES:
:CUSTOM_ID: 53-open-file-status-flags
:END:
* 5.4 Relationship Between File Descriptors and Open Files
:PROPERTIES:
:CUSTOM_ID: 54-relationship-between-file-descriptors-and-open-files
:END:
* 5.5 Duplicating File Descriptors
:PROPERTIES:
:CUSTOM_ID: 55-duplicating-file-descriptors
:END:
* 5.6 File I/O at a Specified Offset: pread() and pwrite
:PROPERTIES:
:CUSTOM_ID: 56-file-io-at-a-specified-offset-pread-and-pwrite
:END:
* 5.7 Scatter-Gather I/O: readv() and writev
:PROPERTIES:
:CUSTOM_ID: 57-scatter-gather-io-readv-and-writev
:END:
* 5.8 Truncating a File: truncate() and ftruncate
:PROPERTIES:
:CUSTOM_ID: 58-truncating-a-file-truncate-and-ftruncate
:END:
* 5.9 Nonblocking I/O
:PROPERTIES:
:CUSTOM_ID: 59-nonblocking-io
:END:
* 5.10 I/O on Large Files
:PROPERTIES:
:CUSTOM_ID: 510-io-on-large-files
:END:
* 5.11 The /dev/fd Directory
:PROPERTIES:
:CUSTOM_ID: 511-the-devfd-directory
:END:
* 5.12 Creating Temporary Files
:PROPERTIES:
:CUSTOM_ID: 512-creating-temporary-files
:END:
* 5.13 Summary
:PROPERTIES:
:CUSTOM_ID: 513-summary
:END:
* 5.14 Exercises
:PROPERTIES:
:CUSTOM_ID: 514-exercises
:END:
        FILE I/O: FURTHER DETAILS
...
scheduling of the two processes means that this is a race condition.
 ...
 open(..., O_WRONLY);
  ...
  open(..., O_WRONLY                                        Key
    | O_CREAT, ...);                           Executing          Waiting
             open() succeeds                       on CPU         for CPU
...
descriptors.
          Process A                  Open file table                      I-node table
    File descriptor table            (system-wide)                       (system-wide)
             fd  file              file    status   inode                file file
                                                                                          ...
           flags ptr              offset    flags     ptr               type locks
    fd 0                      0
    fd 1                                                          224
    fd 2                     23
...
–––––––––––––––––––––––––––––––––––––––––––––––––––––––––– fileio/t_readv.c
                                                              ...
                                                              File I/O: F urther Detai ls   101
           Gather output
...
5.9   Nonblocking I/O
      ...
      employed (Section 55.4).
                                                                             ...
                                                                             File I/O: F urther Detai ls   103
...
main(int argc, char *argv[])
{
    ...
    off64_t off;
                                                                      ...
                                                                      File I/O: F urther Detai ls   105
...
5.11   The /dev/fd Directory
       ...
       other flags, such as O_CREAT, is meaningless (and ignored) in this context.
                    ...
                    held open by a process.
                                                                           ...
                                                                           File I/O: F urther Detai ls   107
...
5.13   Summary
       ...
       other’s output.
                                                                        ...
                                                                        File I/O: F urther Detai ls   109
...
5-7.   Implement readv() and writev() using read(), write(), and suitable functions from the
       malloc package (Section 7.1.2).




                                                                        File I/O: F urther Detai ls   111
