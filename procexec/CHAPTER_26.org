...
#+DESCRIPTION: Chapter 26 The Linux Programming Interface by Michael Kerrisk
#+STARTUP: showall

* Table Of Contents                                                   :toc_2:
- [[#261-waiting-on-a-child-process][26.1 Waiting on a Child Process]]
  - [[#2611-the-wait-system-call][26.1.1 The wait() System Call]]
  - [[#2612-the-waitpid-system-call][26.1.2 The waitpid() System Call]]
  - [[#2613-the-wait-status-value][26.1.3 The Wait Status Value]]
  - [[#2614-process-termination-from-a-signal-handler][26.1.4 Process Termination from a Signal Handler]]
  - [[#2615-the-waitid-system-call][26.1.5 The waitid() System Call]]
  - [[#2616-the-wait3-and-wait4-system-calls][26.1.6 The wait3() and wait4() System Calls]]
- [[#262-orphans-and-zombies][26.2 Orphans and Zombies]]
- [[#263-the-sigchld-signal][26.3 The SIGCHLD Signal]]
  - [[#2631-establishing-a-handler-for-sigchld][26.3.1 Establishing a Handler for SIGCHLD]]
  - [[#2632-delivery-of-sigchld-for-stopped-children][26.3.2 Delivery of SIGCHLD for Stopped Children]]
  - [[#2633-ignoring-dead-child-processes][26.3.3 Ignoring Dead Child Processes]]
- [[#264-summary][26.4 Summary]]
- [[#265-exercises][26.5 Exercises]]

* 26.1 Waiting on a Child Process
:PROPERTIES:
:CUSTOM_ID: 261-waiting-on-a-child-process
:END:

** 26.1.1 The wait() System Call
:PROPERTIES:
:CUSTOM_ID: 2611-the-wait-system-call
:END:

** 26.1.2 The waitpid() System Call
:PROPERTIES:
:CUSTOM_ID: 2612-the-waitpid-system-call
:END:

** 26.1.3 The Wait Status Value
:PROPERTIES:
:CUSTOM_ID: 2613-the-wait-status-value
:END:

** 26.1.4 Process Termination from a Signal Handler
:PROPERTIES:
:CUSTOM_ID: 2614-process-termination-from-a-signal-handler
:END:

** 26.1.5 The waitid() System Call
:PROPERTIES:
:CUSTOM_ID: 2615-the-waitid-system-call
:END:

** 26.1.6 The wait3() and wait4() System Calls
:PROPERTIES:
:CUSTOM_ID: 2616-the-wait3-and-wait4-system-calls
:END:

* 26.2 Orphans and Zombies
:PROPERTIES:
:CUSTOM_ID: 262-orphans-and-zombies
:END:

* 26.3 The SIGCHLD Signal
:PROPERTIES:
:CUSTOM_ID: 263-the-sigchld-signal
:END:

** 26.3.1 Establishing a Handler for SIGCHLD
:PROPERTIES:
:CUSTOM_ID: 2631-establishing-a-handler-for-sigchld
:END:

** 26.3.2 Delivery of SIGCHLD for Stopped Children
:PROPERTIES:
:CUSTOM_ID: 2632-delivery-of-sigchld-for-stopped-children
:END:

** 26.3.3 Ignoring Dead Child Processes
:PROPERTIES:
:CUSTOM_ID: 2633-ignoring-dead-child-processes
:END:

* 26.4 Summary
:PROPERTIES:
:CUSTOM_ID: 264-summary
:END:

* 26.5 Exercises
:PROPERTIES:
:CUSTOM_ID: 265-exercises
:END:
...
             #include <sys/wait.h>
           ...
           2.      If status is not NULL, information about how the child terminated is returned in
                   ...
                   in Section 26.1.3.
           3.      The kernel adds the process CPU times (Section 10.7) and resource usage statistics
...
––––––––––––––––––––––––––––––––––––––––––––––––––––– procexec/multi_wait.c
                                                                ...
                                                                M o n it o ri n g C h i ld P r o c e ss es   543
26.1.2     The waitpid() System Call
...
26.1.3   The Wait Status Value
         ...
         Figure 26-1: Value returned in the status argument of wait() and waitpid()
                                                                            ...
                                                                            M o n it o ri n g C h i ld P r o c e ss es   545
...
SIGCONT signals to the child:
    ...
    $ Child started with PID = 15871
                                                            ...
                                                            M o n it o ri n g C h i ld P r o c e ss es   547
                   kill -STOP 15871
...
26.1.4   Process Termination from a Signal Handler
         ...
         then the child’s signal handler should first disestablish itself, and then raise the
                                                                        ...
                                                                        M o n it o ri n g C h i ld P r o c e ss es   549
...
provides a method of distinguishing the two possibilities: we can check whether the
                                                                     ...
                                                                     M o n it o ri n g C h i ld P r o c e ss es   551
...
26.2   Orphans and Zombies
       ...
       put things another way, in such applications, a parent process should perform
                                                                        ...
                                                                        M o n it o ri n g C h i ld P r o c e ss es   553
...
26.3.1   Establishing a Handler for SIGCHLD
         ...
         are not queued. Consequently, if a second and third child terminate in quick
                                                                          ...
                                                                          M o n it o ri n g C h i ld P r o c e ss es   555
...
q           printf("%s handler: Reaped child %ld - ", currTime("%T"),
        ...
        if (childPid == -1 && errno != ECHILD)
            errMsg("waitpid");
                                                                    ...
                                                                    M o n it o ri n g C h i ld P r o c e ss es   557
...
26.3.3       Ignoring Dead Child Processes
             ...
             setting the disposition of SIGCHLD to SIG_IGN does remove existing zombie children.
                                                                              ...
                                                                              M o n it o ri n g C h i ld P r o c e ss es   559
...
26.4   Summary
       ...
       Refer to the sources of further information listed in Section 24.6.
                                                                     ...
                                                                     M o n it o ri n g C h i ld P r o c e ss es   561
26.5       Exercises
