...
z   The ac_comm field records the name of the last command (program file) exe-
    ...
    other UNIX implementations, this field is limited to 8 characters.
                                       ...
                                       P ro ce s s Cre ati o n an d P ro g ram E xe cu t i o n i n Mo re D e tai l   593
...
Finally, we use the program in Listing 28-2 to view the contents of the accounting file:
    ...
    t_fork    ----      0         mtk        2010-07-23 17:21:36                      0.00         3.01
                                                ...
                                                P ro ce s s Cre ati o n an d P ro g ram E xe cu t i o n i n Mo re D e tai l   595
...
kernel configuration option must be enabled before building the kernel.
                                             ...
                                             P ro ce s s Cre ati o n an d P ro g ram E xe cu t i o n i n Mo re D e tai l   597
...
proc(5) manual page for further details.)
                                           ...
                                           P ro ce s s Cre ati o n an d P ro g ram E xe cu t i o n i n Mo re D e tai l   599
...
w       fd = open("/dev/null", O_RDWR);       /* Child will close this fd */
        if (fd == -1)
            errExit("open");
                                             ...
                                             P ro ce s s Cre ati o n an d P ro g ram E xe cu t i o n i n Mo re D e tai l   601
...
28.2.1   The clone() flags Argument
         ...
         share the same open file descriptors.
                                                     ...
                                                     P ro ce s s Cre ati o n an d P ro g ram E xe cu t i o n i n Mo re D e tai l   603
...
CLONE_THREAD is specified. This corresponds to further POSIX threads requirements;
                                          ...
                                          P ro ce s s Cre ati o n an d P ro g ram E xe cu t i o n i n Mo re D e tai l   605
...
cal and is not permitted.
                                          ...
                                          P ro ce s s Cre ati o n an d P ro g ram E xe cu t i o n i n Mo re D e tai l   607
...
28.2.2   Extensions to waitpid() for Cloned Children
         ...
         __WCLONE
                    ...
                    specified.
                                                    ...
                                                    P ro ce s s Cre ati o n an d P ro g ram E xe cu t i o n i n Mo re D e tai l   609
...
28.3       Speed of Process Creation
           ...
           Table 28-3: Time required to create 100,000 processes using fork(), vfork(), and clone()
            ...
            fork()                   22.27        4544       26.38       4135      126.93       1276
                                     (7.99)                  (8.98)                (52.55)
            vfork()                   3.52       28955        3.55       28621       3.53      28810
                                     (2.49)                  (2.50)                 (2.51)
            clone()                   2.97       34333        2.98       34217       2.93      34688
                                     (2.14)                  (2.13)                 (2.10)
            fork() + exec()         135.72         764      146.15        719      260.34        435
                                    (12.39)                 (16.69)                (61.86)
            vfork() + exec()        107.36         969      107.81        964      107.97        960
                                     (6.27)                  (6.35)                 (6.38)
...
z   Because the times required for process creation are typically much smaller
    than those required for an exec(), the differences are much less marked if a
                                           ...
                                           P ro ce s s Cre ati o n an d P ro g ram E xe cu t i o n i n Mo re D e tai l   611
...
Table 28-4: Effect of exec() and fork() on process attributes
 ...
 Environment variables              See       Yes       putenv(), setenv(); direct modification of environ.
                                    notes               Overwritten by execle() and execve() and preserved
                                                        by remaining exec() calls.
 Memory mappings                    No        Yes;      mmap(), munmap(). A mapping’s MAP_NORESERVE
                                              see       flag is inherited across fork(). Mappings that have
                                              notes     been marked with madvise(MADV_DONTFORK)
...
Table 28-4: Effect of exec() and fork() on process attributes (continued)
 ...
 Effective and saved set IDs        See        Yes      setuid(), setgid(), and related calls. Chapter 9
                                    notes               explains how exec() affects these IDs.
 Supplementary group IDs            Yes        Yes      setgroups(), initgroups().
 Files, file I/O, and directories
 Open file descriptors              See        Yes      open(), close(), dup(), pipe(), socket(), and
                                    notes               so on. File descriptors are preserved
 ...
 Close-on-exec flag                 Yes        Yes      fcntl(F_SETFD).
                                    (if off)
 ...
 Asynchronous I/O operations        See        No       aio_read(), aio_write(), and related calls.
                                    notes               Outstanding operations are canceled
                                                        during an exec().
 Directory streams                  No         Yes;     opendir(), readdir(). SUSv3 states that child gets a
                                               see      copy of parent’s directory streams, but these
                                               notes    copies may or may not share the directory
 ...
 Signals
 Signal dispositions                See        Yes      signal(), sigaction(). During an exec(), signals
                                    notes               with dispositions set to default or ignore are
 ...
 Alternate signal stack             No         Yes      sigaltstack().
                                                         ...
                                                         P ro ce s s Cre ati o n an d P ro g ram E xe cu t i o n i n Mo re D e tai l   613
Table 28-4: Effect of exec() and fork() on process attributes (continued)
 ...
 POSIX threads
 Threads                            No        See       During fork(), only calling thread is replicated in
                                              notes     child.
 Thread cancelability state         No        Yes       After an exec(), the cancelability type and state
 and type                                               are reset to PTHREAD_CANCEL_ENABLE and
                                                        PTHREAD_CANCEL_DEFERRED, respectively
 Mutexes and                        No        Yes       See Section 33.3 for details of the treatment of
 condition variables                                    mutexes and other thread resources during
                                                        fork().
 ...
 System V shared                    No        Yes       shmat(), shmdt().
 memory segments
 ...
 POSIX unnamed semaphores           No        See       sem_init() and related calls. If semaphores are in
                                              notes     a shared memory region, then child shares
                                                        semaphores with parent; otherwise, child has its
                                                        own copy of the semaphores.
 System V semaphore                 Yes       No        semop(). See Section 47.8.
 adjustments
 File locks                         Yes       See       flock(). Child inherits a reference to the same
                                              notes     lock as parent.
 Record locks                       See       No        fcntl(F_SETLK). Locks are preserved across exec()
                                    notes               unless a file descriptor referring to the file is
...
Table 28-4: Effect of exec() and fork() on process attributes (continued)
 ...
 Linux-specific
 File-system IDs                    See       Yes       setfsuid(), setfsgid(). These IDs are also changed any
                                    notes               time the corresponding effective IDs are
                                                        changed.
 timerfd timers                     Yes       See       timerfd_create(); child inherits file descriptors
                                              notes     referring to same timers as parent.
 Capabilities                       See       Yes       capset(). The handling of capabilities during an
                                    notes               exec() is described in Section 39.5.
 Capability bounding set            Yes       Yes
 Capabilities securebits flags      See       Yes       All securebits flags are preserved during
                                    notes               an exec() except SECBIT_KEEP_CAPS, which
                                                        is always cleared.
 ...
 File leases                        Yes       See       fcntl(F_SETLEASE). Child inherits a reference to
                                              notes     the same lease as parent.
 Directory change notifications     Yes       No        The dnotify API, available via fcntl(F_NOTIFY).
 prctl(PR_SET_DUMPABLE)             See       Yes       During an exec(), the PR_SET_DUMPABLE flag is set,
                                    notes               unless execing a set-user-ID or set-group-ID
 ...
 coredump_filter                    Yes       Yes       See Section 22.1.
                                                         ...
                                                         P ro ce s s Cre ati o n an d P ro g ram E xe cu t i o n i n Mo re D e tai l   615
28.5       Summary
