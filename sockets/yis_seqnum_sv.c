/* =========================================================================
 * Created on: <Mon Apr 13 23:01:01 +01 2026>
 * Time-stamp: <Fri Apr 17 16:52:20 +01 2026 by owner>
 * Author    : owner
 * Desc      : ~/coding/c_prog/tlpi/sockets/yis_seqnum_sv.c -
 *
 * See [[file:is_seqnum_sv.c]], [[file:yis_seqnum_cl.c]]
 * ========================================================================= */
#define _GNU_SOURCE
...
#define WHOLE_NI_BUF ((((NI_MAXHOST + NI_MAXSERV) / 4096) * 4096) + 4096)
  ...
  for (;;) {
    ...
    if ((rc = yreadLine(connfd, reqLenStr, INT_LEN)) <= 0) {
      if (rc < 0)
    ...
    if (rc == TGN_FAILURE) { /* Parsing client data failed: Drop client and continue */
      if (close(connfd) == -1)
        systmErr("close() failed");
    ...
    if (write(connfd, seqNumStr, strlen(seqNumStr)) != (ssize_t)strlen(seqNumStr)) {
      if (errno == EPIPE)
