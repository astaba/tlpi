...
maximum number of simultaneous supplementary group IDs for a process
                                                               ...
                                                               S y s t e m L i m i t s a n d O p ti o ns   213
       ...
       run time.
           ...
           long sysconf(int name);
                                         ...
                                         or –1 if limit is indeterminate or an error occurred
                                                                       ...
                                                                       S y s t e m L i m i t s a n d O p ti o ns   215
...
11.3   Retrieving File-Related Limits (and Options) at Run Time
       ...
       a process is running.
                                                                            ...
                                                                            S y s t e m L i m i t s a n d O p ti o ns   217
...
11.5   System Options
       ...
       _POSIX_ or _XOPEN_).
                                                                      ...
                                                                      S y s t e m L i m i t s a n d O p ti o ns   219
...
       Table 11-3: Selected SUSv3 options
        ...
        (_SC_ASYNCHRONOUS_IO)
        _POSIX_CHOWN_RESTRICTED         Only privileged processes can use chown() and                                   *
        (_PC_CHOWN_RESTRICTED)          fchown() to change the user ID and group ID of a
                                        file to arbitrary values (Section 15.3.2)
        _POSIX_JOB_CONTROL              Job Control (Section 34.7)                                                      +
        (_SC_JOB_CONTROL)
        _POSIX_MESSAGE_PASSING          POSIX Message Queues (Chapter 52)
        (_SC_MESSAGE_PASSING)
        _POSIX_PRIORITY_SCHEDULING      Process Scheduling (Section 35.3)
        (_SC_PRIORITY_SCHEDULING)
        _POSIX_REALTIME_SIGNALS         Realtime Signals Extension (Section 22.8)
        (_SC_REALTIME_SIGNALS)
        _POSIX_SAVED_IDS                Processes have saved set-user-IDs and saved                                     +
        (none)                          set-group-IDs (Section 9.4)
        _POSIX_SEMAPHORES               POSIX Semaphores (Chapter 53)
        (_SC_SEMAPHORES)
        _POSIX_SHARED_MEMORY_OBJECTS    POSIX Shared Memory Objects (Chapter 54)
        (_SC_SHARED_MEMORY_OBJECTS)
        _POSIX_THREADS                  POSIX Threads
        (_SC_THREADS)
        _XOPEN_UNIX                     The XSI extension is supported (Section 1.3.4)
        (_SC_XOPEN_UNIX)
...
11.6   Summary
       ...
       ad hoc techniques to determine the limit to which an application should adhere.
                                                                            ...
                                                                            S y s t e m L i m i t s a n d O p ti o ns       221
           Further information
...
11.7       Exercises
  11-1.    Try running the program in Listing 11-1 on other UNIX implementations if you
           have access to them.
