...
usage(char *pname)
{
    ...
    fprintf(stderr, "\t'user' is the user with whose credentials the\n"
      "\t\tprogram is to be launched\n"
      "\t'cap,...' is the set of capabilities with which the\n"
      "\t\tprogram is to be launched\n"
      "\t'cmd' and 'arg...' specify the command plus arguments\n"
      "\t\tfor the program that is to be launched\n"
      "\n"
      "\tOptions:\n"
      "\t    -A  Raise the specified capabilities only in the inheritable set\n"
      "\t\t(and not in ambient set) before launching 'cmd'\n");
    exit(EXIT_FAILURE);
}
